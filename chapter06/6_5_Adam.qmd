...
# Plotting function
draw_function <- function(phi0mesh, phi1mesh, loss_function, opt_path = NULL, title = NULL, parameter = NULL) {
  df <- expand.grid(phi0 = phi0mesh, phi1 = phi1mesh)
  df$loss <- c(loss_function)
  p <- ggplot() +
    xlim(-1, 1) +
    ylim(-1, 1) +
    ...
    geom_contour(
      ...
      linewidth = 0.25)
  if (!is.null(opt_path)) {
    p <- p + geom_point(aes(x = opt_path[1,], y = opt_path[2,]), color = "red3")
    p <- p + geom_text(aes(x = opt_path[1,1:5], y = opt_path[2,1:5], label = 1:5),
                       hjust = 0,          # Align text to the left of its point (0)
                       nudge_x = 0.01,      # Shift text slightly to the right
                       vjust = 0.5,
                       angle = 0,
                       color = "darkblue")
    p <- p + geom_path(aes(x = opt_path[1,], y = opt_path[2,]), color = "red3")
  }
  if (!is.null(title)) {
    # p <- p + ggtitle(title) + theme(plot.title = element_text(hjust = 0.5)) 
    p <- p + annotate("text", x = 0.95, y = -0.90, hjust = 1, label = title, color = "black")
  }
  if (!is.null(parameter)) {
    p <- p + annotate("text", x = 0.95, y = -0.95, hjust = 1, label = parameter, color = "black")
  }
  p
}
```
...
parameter <- expression(alpha*" = 0.05")
draw_function(phi0mesh, phi1mesh, loss_function, grad_path1, "Gradient descent", parameter)
grad_path2 <- grad_descent(start_posn, n_steps = 40, alpha = 1.0)
parameter <- expression(alpha*" = 1.0")
draw_function(phi0mesh, phi1mesh, loss_function, grad_path2, "Gradient descent", parameter)
```
...
```{r}
#| label: test-normalized_gradients
#| fig-asp: 1
...
parameter <- expression(alpha*" = 0.05")
draw_function(phi0mesh, phi1mesh, loss_function, grad_path1, "Normalized gradients", parameter)
```
...
adam <- function(start_posn, n_steps, alpha, beta = 0.9, gamma = 0.99, epsilon = 1e-20) {
  ...
  for (c_step in 1:n_steps) {
    ...
    # Replace this line:
    grad_path[,c_step+1] <- grad_path[,c_step] - alpha * m_tilde / (sqrt(v_tilde)+epsilon)
  }
  grad_path
}   
```
...
parameter <- expression(alpha*" = 0.05,"*beta*" = 0.9,"*gamma*" = 0.99")
draw_function(phi0mesh, phi1mesh, loss_function, grad_path1, "Adam", parameter)
```