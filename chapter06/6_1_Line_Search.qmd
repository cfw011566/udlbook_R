...
draw_function <- function(loss_function, a = NULL, b = NULL, c = NULL, d = NULL) {
  ...
  p <- ggplot() +
    ...
    geom_line(aes(x = phi_plot, y = loss_function(phi_plot)), color = "red3")
  
  if (!is.null(a) && !is.null(b) && !is.null(c) && !is.null(d)) {
    p <- p +
      geom_vline(xintercept = a, linetype = 1, color = "skyblue3") +
      geom_vline(xintercept = b, linetype = 1, color = "skyblue3") +
      geom_vline(xintercept = c, linetype = 1, color = "skyblue3") +
      geom_vline(xintercept = d, linetype = 1, color = "skyblue3")
  }
  p
}
...
line_search <- function(loss_function, thresh = 0.0001, max_iter = 10, draw_flag = FALSE) {
  ...
  while (abs(b-c) > thresh && n_iter < max_iter) {
    ...
    if (draw_flag) {
      print(draw_function(loss_function, a, b, c, d))
    }
