...
# all_losses <- outer(phi0_range, phi1_range, function(p0, p1) { compute_loss(x, y, p0, p1)})
  ...
  color_rgb <- col2rgb("skyblue") / 255
  points_color <- rgb(red = color_rgb[1], green = color_rgb[2], blue = color_rgb[3], alpha = seq(0.5, 1, length.out=10))
...
$$
\begin{aligned}
\frac{\partial L}{\partial\phi_0} &= \frac{L[\phi_0+\delta,\phi_1] - L[\phi_0,\phi_1]}{\delta} \\
\frac{\partial L}{\partial\phi_1} &= \frac{L[\phi_0,\phi_1+\delta] - L[\phi_0,\phi_1]}{\delta}
\end{aligned}
$$
...
n_steps <- 10
phi_all <- matrix(0, nrow = 2, ncol = n_steps)
phi_all[1,1] <- 1.6
phi_all[2,1] <- -0.5
...
for (c_step in 1:(n_steps-1)) {
  ...
  draw_model(data, model, phi_all[,c_step+1], sprintf("Iteration %d, loss = %f", c_step+1, loss))
}
