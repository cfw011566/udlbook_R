...
# all_losses <- outer(phi0_range, phi1_range, function(p0, p1) { compute_loss(x, y, p0, p1)})
  ...
  filled.contour(
    ...
    xlab = expression(paste("Offset, ", phi[0])),
    ylab = expression(paste("Frequency, ", phi[1])),
    plot.axes = {
      axis(1)
      axis(2)
...
stochastic_gradient_descent_step <- function(phi, data, alpha, batch_size) {
  ...
  random_data <- data[,sample(ncol(data))]
  batch_data <- data[,1:batch_size]
...
for (c_step in 1:n_steps) {
  # Do gradient descent step
  phi_all[,c_step+1] <- stochastic_gradient_descent_step(phi_all[,c_step], data, alpha = 0.8, batch_size = 10)
  # Measure loss and draw model every 8th step
  if (c_step %% 8 == 0) {
    loss <- compute_loss(data[1,], data[2,], model, phi_all[,c_step+1])
