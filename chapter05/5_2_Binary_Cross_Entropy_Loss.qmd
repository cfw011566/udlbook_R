...
plot_binary_classification <- function(x_model, out_model, lambda_model, x_data = NULL, y_data = NULL, title = NULL) {
  ...
  x_model <- c(x_model)
  out_model <- c(out_model)
  lambda_model <- c(lambda_model)
  ...
  p1 <- ggplot() +
    ...
    geom_line(mapping = aes(x = x_model, y = out_model), color = "skyblue3", linewidth = 0.75) +
    ggtitle(title) +
  ...
  p2 <- ggplot() +
    ...
    geom_line(mapping = aes(x = x_model, y = lambda_model), color = "skyblue3", linewidth = 0.75) +
    ggtitle(title) +
    theme(plot.title = element_text(hjust = 0.5)) 
  if (!is.null(x_data)) {
    p2 <- p2 +
...
bernoulli_distribution <- function(y, lambda_param) {
  ...
  (1.0 - lambda_param)^(1.0 - y) * lambda_param^y
}
...
compute_likelihood <- function(y_train, lambda_param) {
  ...
  # Replace the line below
  prod(bernoulli_distribution(y_train, lambda_param))
}
...
compute_negative_log_likelihood <- function(y_train, lambda_param) {
  ...
  # Replace the line below
  sum(-log(bernoulli_distribution(y_train, lambda_param)))
}
...
for (count in 1:length(beta_1_vals)) {
  ...
  if ((count - 1) %% 20 == 0) {
    ...
    model_out <- shallow_nn(x_model, params$beta_0, params$omega_0, params$beta_1, params$omega_1)
    lambda_model <- sigmoid(model_out)
    p <- p / plot_binary_classification(x_model, model_out, lambda_model, x_train, y_train, title = sprintf("beta_1[1]=%3.3f", params$beta_1[1,1]))
  }
}
p
```
...
ggplot() +
  xlab("beta_1") +
  scale_y_continuous(
    limits = c(0, 3.5e-4), breaks = seq(0, 3.5e-4, 5e-5),
    sec.axis = sec_axis(~.*scale+5, name = "negative log likelihood", breaks = seq(0, 35, 5)))+
  geom_line(aes(x = beta_1_vals, y = likelihoods), color = likelihood_color) +
  geom_line(aes(x = beta_1_vals, y = (nlls-5)/scale), color = nll_color) +
...
model_out <- shallow_nn(x_model, params$beta_0, params$omega_0, params$beta_1, params$omega_1)
lambda_model <- sigmoid(model_out)
plot_binary_classification(x_model, model_out, lambda_model, x_train, y_train, title = sprintf("beta_1=%3.3f", params$beta_1[1,1]))
```
