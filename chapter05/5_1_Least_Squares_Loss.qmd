...
plot_univariate_regression <- function(x_model, y_model, x_data = NULL, y_data = NULL, sigma_model = NULL, title = NULL) {
  par(xaxs = "i", yaxs = "i")
  plot(0, type = "n", las = 1, xlim = c(0, 1), ylim = c(-1, 1),
       # xaxp = c(-1, 1, 8),
       yaxp = c(-1, 1, 8),
       xlab = expression("Input, "*x),
       ylab = expression("Output, "*y)
  )
  if (!is.null(title)) {
    title(title)
  }
  if (!is.null(sigma_model)) {
    polygon(c(x_model, rev(x_model)), c(y_model+2*sigma_model, rev(y_model)-2*sigma_model), col = "lightgray", border = NA)
  }
  lines(x_model, y_model, col = "skyblue3", lwd = 2)
  if (!is.null(x_data)) {
    points(x_data, y_data, pch = 19, cex = 1.25)
  }
}
```
...
normal_distribution <- function(y, mu, sigma) {
  ...
  prob <- exp(-(y-mu)^2 / 2.0 / sigma^2) / sqrt(2*pi*sigma^2)
}
...
par(xaxs = "i", yaxs = "i")
plot(0, type = "n", las = 1, xlim = c(-5, 5), ylim = c(0, 1),
     xlab = expression("Input, "*y),
     ylab = expression("Probability "*Pr(y))
)
lines(y_gauss, gauss_prob, col = "skyblue3", lwd = 2)
...
compute_likelihood <- function(y_train, mu, sigma) {
  ...
  # Replace the line below
  likelihood <- prod(normal_distribution(y_train, mu, sigma))
}
...
compute_negative_log_likelihood <- function(y_train, mu, sigma) {
  ...
  # Replace the line below
  nll <- sum(-log(normal_distribution(y_train, mu, sigma)))
}
...
compute_sum_of_squares <- function(y_train, y_pred) {
  ...
  # Replace the line below
  sum_of_squares = sum((y_train - y_pred)^2)
}
...
for (count in 1:length(beta_1_vals)) {
  ...
  if ((count-1) %% 20 == 0) {
    # Run the model to get values to plot and plot it.
    y_model <- shallow_nn(x_model, params$beta_0, params$omega_0, params$beta_1, params$omega_1)
    plot_univariate_regression(x_model, y_model, x_train, y_train, sigma_model = sigma, title = sprintf("beta1=%.3f", params$beta_1[1,1]))
  }
}
```
...
mtext("likelihood", las = 0, side = 2, line = 3, col = likelihood_color)
...
par(new = FALSE)
plot(beta_1_vals, sum_squares, type = "l", lwd = 2, col = "skyblue3",
     xlab = "beta_1[0]", ylab = "sum of squqares")
```
...
y_model <- shallow_nn(x_model, params$beta_0, params$omega_0, params$beta_1, params$omega_1)
title <- sprintf("beta1=%3.3f", params$beta_1[1,1])
plot_univariate_regression(x_model, y_model, x_train, y_train, sigma_model = sigma, title = title)
```
...
for (count in 1:length(sigma_vals)) {
  ...
  if ((count-1) %% 20 == 0) {
    # Run the model to get values to plot and plot it.
    y_model <- shallow_nn(x_model, params$beta_0, params$omega_0, params$beta_1, params$omega_1)
    plot_univariate_regression(x_model, y_model, x_train, y_train, sigma_model = sigma, title = sprintf("sigma=%.3f", sigma))
  }
}
```
...
plot(sigma_vals, likelihoods, type = "l", lwd = 2, col = likelihood_color,
#     axes = FALSE,
     xlab = "sigma", ylab = "",
     ylim = range(likelihoods))
#axis(side = 1, at = pretty(range(sigma_vals)))

mtext("likelihood", las = 0, side = 2, line = 3, col = likelihood_color)
axis(side = 2, at = pretty(range(likelihoods)), col.axis = likelihood_color, col.ticks = likelihood_color)

par(new = TRUE)
plot(sigma_vals, nlls, type = "l", lwd = 2, col = nll_color,
     axes = FALSE, xlab = "", ylab = "",
     ylim = range(nlls))
axis(side = 4, at = pretty(range(nlls)), col.axis = nll_color, col.ticks = nll_color)
mtext("negative log likelihood", las = 0, side = 4, line = 2, col = nll_color)

abline(v = sigma_vals[which.max(likelihoods)], lty = 3, col = nll_color)

par(new = FALSE)
plot(sigma_vals, sum_squares, type = "l", lwd = 2, col = "skyblue3",
     xlab = "sigma", ylab = "sum of squqares")
```
