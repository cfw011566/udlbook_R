...
for (count in 1:length(beta_1_vals)) {
  ...
  if ((count-1) %% 20 == 0) {
    # Run the model to get values to plot and plot it.
    y_model <- shallow_nn(x_model, params$beta_0, params$omega_0, params$beta_1, params$omega_1)
    p <- plot_univariate_regression(x_model, y_model, x_train, y_train, sigma_model = sigma, title = sprintf("beta1=%.3f", params$beta_1[1,1]))
    print(p)
  }
}
```
...
for (count in 1:length(sigma_vals)) {
  ...
  if ((count-1) %% 20 == 0) {
    # Run the model to get values to plot and plot it.
    y_model <- shallow_nn(x_model, params$beta_0, params$omega_0, params$beta_1, params$omega_1)
    p <- plot_univariate_regression(x_model, y_model, x_train, y_train, sigma_model = sigma, title = sprintf("sigma=%.3f", sigma))
    print(p)
  }
}
```
