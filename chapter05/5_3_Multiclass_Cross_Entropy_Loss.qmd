...
plot_multiclass_classification <- function(x_model, out_model, lambda_model, x_data = NULL, y_data = NULL, title = NULL) {
  ...
  plot(0, type = "n", xlim = c(0, 1), ylim = c(-4, 4),
       # xaxp = c(0, 1, 8),
       yaxp = c(-4, 4, 8),
       xlab = expression("Input, "*x),
       ylab = "Model output"
  )
  lines(x_model, out_model[1,], lwd = 2, col = "red3")
  lines(x_model, out_model[2,], lwd = 2, col = "green3")
  lines(x_model, out_model[3,], lwd = 2, col = "blue3")

  if (!is.null(title)) {
    title(title)
  }
  
  plot(0, type = "n", xlim = c(0, 1), ylim = c(-0.1, 1.05),
       xlab = expression("Input, "*x),
       ylab = expression(lambda*" or Pr(y=1|x)")
  )
  lines(x_model, lambda_model[1,], lwd = 2, col = "red3")
  lines(x_model, lambda_model[2,], lwd = 2, col = "green3")
  lines(x_model, lambda_model[3,], lwd = 2, col = "blue3")
  if (!is.null(title)) {
    title(title)
  }
  if (!is.null(x_data)) {
    ...
    points(x_data, y_coord, pch = 19, col = point_colors)
#    for (i in 1:length(x_data)) {
#      if (y_data[i] == 0) {
#        points(x_data[i], -0.05, pch = 19, col = "red3")
#      }
#      if (y_data[i] == 1) {
#        points(x_data[i], -0.05, pch = 19, col = "green3")
#      }
#      if (y_data[i] == 2) {
#        points(x_data[i], -0.05, pch = 19, col = "blue3")
#      }
#    }
  }
}
...
for (count in 1:length(beta_1_vals)) {
  ...
  if ((count-1) %% 20 == 0) {
    ...
    model_out <- shallow_nn(x_model, params$beta_0, params$omega_0, params$beta_1, params$omega_1)
    lambda_model <- softmax(model_out)
    plot_multiclass_classification(x_model, model_out, lambda_model, x_train, y_train, title = sprintf("beta1[1,1]=%3.3f", params$beta_1[1,1]))
  }
}
```
...
plot(beta_1_vals, likelihoods, type = "l", lwd = 2, col = likelihood_color,
     axes = FALSE,
     xlab = "beta_1[1,1]", ylab = "",
     ylim = range(likelihoods))
#axis(side = 1, #at = pretty(range(beta_1_vals)))
axis(side = 1, at = seq(min(beta_1_vals), max(beta_1_vals, length.out = 8)))
...
model_out <- shallow_nn(x_model, params$beta_0, params$omega_0, params$beta_1, params$omega_1)
lambda_model <- softmax(model_out)
plot_multiclass_classification(x_model, model_out, lambda_model, x_train, y_train, title = sprintf("beta1[0,0]=%3.3f", params$beta_1[1,1]))
```
