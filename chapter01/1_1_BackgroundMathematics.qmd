---
title: "1.1 Background Mathematics"
author: "CF Wang"
date: 2026-01-17
format: html
---
...
ggplot(data.frame(x = x, y = y), aes(x = x, y = y)) +
  geom_line(linewidth = 0.5, color = "red3") +
...
```{r}
#| label: simulate-np.meshgrid
#| eval: false

# like np.meshgrid, turn vector into matrix and do calculation on matrix
x11 <- replicate(length(x2), x1)
x22 <- matrix(rep(x2, times = length(x1)),
              nrow = length(x1),
              ncol = length(x2),
              byrow = TRUE)
yy <- linear_function_2D(x11, x22, beta, omega1, omega2)
draw_2D_function(x1, x2, yy)
```
...
linear_function_3D <- function(x1, x2, x3, beta, omega1, omega2, omega3) {
  ...
  beta + omega1 * x1 + omega2 * x2 + omega3 * x3
}
```
...
cat(paste("Individual equations",  sprintf("y1 = %.3f", y1), sprintf("y2 = %.3f", y2), sep = "\n"))
...
cat(paste("Matrix/vector form", sprintf("y1 = %.3f", y_vec[1]), sprintf("y2 = %.3f", y_vec[2]), sep = "\n"))
```
